...
pushd pylith_installer && \
  ...
  ./configure --prefix=${TOP_DIR}/dist --enable-mpi=mpich --enable-cppunit --enable-numpy --enable-proj4 --enable-hdf5 --enable-netcdfpy --enable-cmake --enable-setuptools --enable-nemesis --enable-fiat --enable-pcre --enable-swig --with-fortran=no --enable-force-install --with-make-threads=${MAKE_THREADS} && \
  . ./setup.sh && \
  make deps && \
  popd && \
